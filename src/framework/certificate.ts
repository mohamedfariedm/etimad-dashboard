...
export function useServices(param:string) {
  ...
  return useQuery<any, Error>({queryKey: [routes.Services.index], queryFn: () => client.Certificates.all(param)});
};
...
export const useCreateServices = () => {
  ...
  const {mutate, isPending} = useMutation({
    mutationFn: client.Certificates.create,
    onSuccess() {
...
export const useUpdateServices = () => {
  ...
  const { closeModal } = useModal();
  return useMutation({
    mutationFn: client.Certificates.update,
    onSuccess: () => {
      queryClient.invalidateQueries({queryKey: [routes.Services.index]})
...
export const useDeleteServices = () => {
    const queryClient = useQueryClient();
    return useMutation({
      mutationFn: client.Certificates.delete,
