...
import {
  ServiceFormInput,
  ServiceFormSchema,
} from '@/utils/validators/service_cerform.schema';
import { useCreateServices, useUpdateServices } from '@/framework/certificate';
  ...
  const onSubmit: SubmitHandler<ServiceFormInput> = (data) => {
    ...
    if (initValues) {
      ...
      update({
        ...
        // },
        // brands: data.brands.map((brand) => brand.value),
        attachment: isImageData || initValues.image,
        brand_id: initValues.id,
        type: 'individual',
      });
    } else {
