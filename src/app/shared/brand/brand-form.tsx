...
export default function UpdateCreateBrand({
  ...
  const onSubmit: SubmitHandler<BrandFormInput> = (data) => {
    ...
    console.log(data);
    const selectedType = type || data.type;
    if (!isImageData) {
      setImageError(1);
    } else {
      if (initValues) {
          ...
          return (
            ...
            render={({ field: { value, onChange } }) => (
              <CreatableSelect
                ...
                placeholder={initValues?.type}
                isMulti={false} // إذا كنت تريد حقلًا يسمح باختيار نوع واحد فقط
                // isLoading={loading} // حالة التحميل إذا كان هنالك بيانات قيد التحميل
