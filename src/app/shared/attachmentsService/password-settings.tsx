...
import { useUploadSection } from '@/framework/attachmentsService';
...
export default function PasswordSettingsView({
  ...
  const isPost = formData?.Posts !== undefined;
  const { mutate: updateSection } = useUploadSection();
  const { mutate: updatePost } = useUpdateSection();

  
  const [isLoading, setLoading] = useState(false);
  const [isImageData, setImage] = useState(formData?.children?.map((child: any) => child.attachment) || []);
  const [isPArentImageData, setParentImage] = useState(settings?.sliders ? [...settings?.sliders] : []);
...
{settings?.description?.en && (
  ...
  <FormGroup title="Description EN" className="pt-7 @2xl:pt-9 @3xl:grid-cols-12 @3xl:pt-11">
    ...
    {errors.description?.en?.message && (
      <span className="text-red-500 text-[12px]">{errors.description.en.message}</span>
    )}
  </FormGroup>
  ...
  <FormGroup title="Description AR" className="pt-7 @2xl:pt-9 @3xl:grid-cols-12 @3xl:pt-11">
    ...
    {errors.description?.ar?.message && (
      <span className="text-red-500 text-[12px]">{errors.description.ar.message}</span>
    )}
  </FormGroup>
</>
)}
          ...
          {(settings?.title?.en || settings?.title?.ar ||
           settings?.description?.en || settings?.description?.ar ||
          (settings?.sliders ?? []).length > 0) && (
            <div className="mt-6 flex w-auto items-center justify-end gap-3">
              {/* <Button type="button" variant="outline">
                Cancel
              </Button> */}
              <Button type="submit" variant="solid" className="dark:bg-gray-100 dark:text-white">
                Update
              </Button>
            </div>
          )}
