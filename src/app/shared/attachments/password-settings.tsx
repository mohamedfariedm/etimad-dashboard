...
export default function PasswordSettingsView({
  ...
  const onSubmit: SubmitHandler<PasswordFormTypes> = (data) => {
    ...
    const hasTitleFromAPI = settings?.title?.en || settings?.title?.ar;
    const hasDescriptionFromAPI = settings?.description?.en || settings?.description?.ar;
    const hasTitleValueFromAPI = settings?.title?.value !== undefined;


    if (hasTitleFromAPI) {
      if (!data.title?.en?.trim()) {
        updatedFields.title = { ...(updatedFields.title || {}), en: TEMP_VALUE };
      } else {
        updatedFields.title = { ...(updatedFields.title || {}), en: data.title.en };
      }
  
      if (!data.title?.ar?.trim()) {
        updatedFields.title = { ...(updatedFields.title || {}), ar: TEMP_VALUE };
      } else {
        updatedFields.title = { ...(updatedFields.title || {}), ar: data.title.ar };
      }
    }

    
    if (hasDescriptionFromAPI) {
      if (!data.description?.en?.trim()) {
        updatedFields.description = { ...(updatedFields.description || {}), en: TEMP_VALUE };
      } else {
        updatedFields.description = { ...(updatedFields.description || {}), en: data.description.en };
      }
  
      if (!data.description?.ar?.trim()) {
        updatedFields.description = { ...(updatedFields.description || {}), ar: TEMP_VALUE };
      } else {
        updatedFields.description = { ...(updatedFields.description || {}), ar: data.description.ar };
      }
    }

    if (hasTitleValueFromAPI || data.title?.value !== undefined) {
      updatedFields.title = { ...(updatedFields.title || {}), value: data.title.value };
    }
